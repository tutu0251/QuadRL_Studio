...
export default function App() {
  ...
  const setWholeCom = useEditorStore((s) => s.setWholeCom);
  const log = useEditorStore((s) => s.log);
  const [projects, setProjects] = useState<string[]>([]);
  const [projectDetails, setProjectDetails] = useState<
    { name: string; hasGeoUrdf: boolean; hasPhysics: boolean }[]
  >([]);
  ...
  const reloadModel = useCallback(
    async (name: string) => {
      ...
      setModel(m);
      const com = await api.robotCom(name);
      setWholeCom(com.com);
    },
    [setModel, setWholeCom]
  );
  ...
  useEffect(() => {
    ...
    api
      ...
      .then(() => {
        setConnected(true);
        log("Connected to Physics Editor API");
      })
      .catch(() => {
        setConnected(false);
        setError("Backend unreachable — start physics-editor on port 8001");
      });
  }, [refreshProjects, log]);
  ...
  const loadProject = async (name: string) => {
    ...
    try {
      ...
      setModel(r.model);
      if (r.hasPhysics) await reloadModel(name);
      else {
        setWholeCom(null);
        log(`Loaded ${name} — File → Import geo URDF to load physics`);
      }
  ...
  const importGeo = async () => {
    ...
    setError(null);
    try {
      const r = await api.importGeo(project);
      setModel(r.model);
      await reloadModel(project);
      log(`Imported geo_${project}.urdf → physics model`);
      await refreshProjects();
    } catch (e) {
      setError(String(e));
    }
  ...
  return (
    <div className="app unity-layout physics-app">
