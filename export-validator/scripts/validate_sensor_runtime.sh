...
usage() {
  cat <<'EOF'
Usage: validate_sensor_runtime.sh <project_name> [--no-build] [--no-generate] [--quiet]
...
quiet = bool(${QUIET})
log = cli_log("sensor") if not quiet else None
if log:
    log_stage(log, f"sensor runtime validation — ${PROJECT}")
