...
def quat_to_rpy(q: Quat) -> tuple[float, float, float]:
    """Convert quaternion to roll-pitch-yaw (XYZ intrinsic / URDF convention)."""
    q = quat_normalize(q)
...
def axis_angle_quat(axis: Vec3, angle: float) -> Quat:
    n = math.sqrt(axis.x ** 2 + axis.y ** 2 + axis.z ** 2)
    if n < 1e-9:
        return Quat(w=1.0)
    ax, ay, az = axis.x / n, axis.y / n, axis.z / n
    s = math.sin(angle / 2)
    return Quat(x=ax * s, y=ay * s, z=az * s, w=math.cos(angle / 2))
...
def vec3_normalize(v: Vec3) -> Vec3:
    n = vec3_norm(v) or 1.0
    return Vec3(x=v.x / n, y=v.y / n, z=v.z / n)


def angle_between(a: Vec3, b: Vec3) -> float:
    na, nb = vec3_normalize(a), vec3_normalize(b)
    dot = max(-1.0, min(1.0, vec3_dot(na, nb)))
