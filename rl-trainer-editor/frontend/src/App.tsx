...
export default function App() {
  ...
  const setProject = useTrainerStore((s) => s.setProject);
  ...
  const [projectDetails, setProjectDetails] = useState<
    { name: string; hasSensor: boolean; hasTrainer: boolean }[]
  >([]);
  ...
  useEffect(() => {
    ...
    api
      ...
      .then(() => {
        setConnected(true);
        log("Connected to RL Trainer API");
      })
      .catch(() => {
        setConnected(false);
        setError("Backend unreachable — start rl-trainer-editor on port 8005");
      });
  }, [refreshProjects, log]);
