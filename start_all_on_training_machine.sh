#!/usr/bin/env bash
# Start all QuadRL Studio backends + frontends on a single machine.
#
# Intended usage: run this on the training machine, then open the UIs from your PC
# using http://<training_machine_ip>:<port>.
set -euo pipefail
...
training_ip() {
  # First IPv4 from hostname -I; fallback to 127.0.0.1.
  local ip
